...
from bl_methods.HA_GAN_master.utils import trim_state_dict_name, inf_train_gen
from bl_methods.HA_GAN_master.volume_dataset import Volume_Dataset_MRI
...
parser.add_argument('--data-dir', type=str, default='/home1/yujiali/dataset/brain_MRI/ADNI/T1/aligned_brain_MNI',
                    help='path to the preprocessed data folder')
parser.add_argument('--exp-name', default='HA_GAN_256', type=str,
                    help='name of the experiment')
parser.add_argument('--fold', default=1, type=int,
                    help='fold number for cross validation')
parser.add_argument('--gpu_ids', default="4", type=str,
                    help='the gpu to use')
...
def main():
    ...
    trainset = Volume_Dataset_MRI(data_dir=args.data_dir, fold=args.fold, num_class=args.num_class)
    ...
    G = Generator(mode='train', latent_dim=args.latent_dim, num_class=args.num_class)
    D = Discriminator(num_class=args.num_class)
    E = Encoder()
    Sub_E = Sub_Encoder(latent_dim=args.latent_dim)
    
    print(cnn_paras_count(G)[1]+cnn_paras_count(D)[1]+cnn_paras_count(E)[1]+cnn_paras_count(Sub_E)[1])
    pdb.set_trace()
    ...
    if args.continue_iter != 0:
        ...
        ckpt_path = '/home1/yujiali/cf_mri_2/bl_methods/HA_GAN_master/checkpoint/'+args.exp_name+'/D_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
        D.load_state_dict(ckpt['model'])
        d_optimizer.load_state_dict(ckpt['optimizer'])
        ckpt_path = '/home1/yujiali/cf_mri_2/bl_methods/HA_GAN_master/checkpoint/'+args.exp_name+'/E_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
        E.load_state_dict(ckpt['model'])
        e_optimizer.load_state_dict(ckpt['optimizer'])
        ckpt_path = '/home1/yujiali/cf_mri_2/bl_methods/HA_GAN_master/checkpoint/'+args.exp_name+'/Sub_E_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
    ...
    summary_writer = SummaryWriter("/home1/yujiali/cf_mri_2/bl_methods/HA_GAN_master/checkpoint/"+args.exp_name)
    
    # save configurations to a dictionary
    with open(os.path.join("/home1/yujiali/cf_mri_2/bl_methods/HA_GAN_master/checkpoint/"+args.exp_name, 'configs.json'), 'w') as f:
    ...
    for iteration in range(args.continue_iter, args.num_iter):
        ...
        #pdb.set_trace()
        crop_idx = np.random.randint(40,256-70) # 256 * 7/8 + 1
        ...
        if iteration%100 == 0:
            ...
            #pdb.set_trace()
            visual_dir = '/home1/yujiali/cf_mri_2/bl_methods/HA_GAN_master/checkpoint/'+args.exp_name+'/visual/volume'
            os.makedirs(visual_dir, exist_ok=True)
            #featmask = np.squeeze((0.5*real_images_crop[0]+0.5).data.cpu().numpy())
            #ants.from_numpy(featmask).to_file(os.path.join(visual_dir, 'iter={}_real.nii.gz'.format(iteration)))
            
            #featmask = np.squeeze((0.5*sub_x_hat_rec[0]+0.5).data.cpu().numpy())
            #ants.from_numpy(featmask).to_file(os.path.join(visual_dir, 'iter={}_rec.nii.gz'.format(iteration)))

            #featmask = np.squeeze((0.5*fake_images[0]+0.5).data.cpu().numpy())
            #ants.from_numpy(featmask).to_file(os.path.join(visual_dir, 'iter={}_fake.nii.gz'.format(iteration)))
            ...
            fake_image = fake_images.squeeze()
            ...
            featmask.to_file(os.path.join(visual_dir, 'test_iter={}.nii.gz'.format(iteration)))
            G.train()
            G.module.mode='train'
            
        if iteration > 10000 and (iteration+1)%5000 == 0:
            torch.save({'model':G.state_dict(), 'optimizer':g_optimizer.state_dict()},
                       '/home1/yujiali/cf_mri_2/bl_methods/HA_GAN_master/checkpoint/'+args.exp_name+'/G_iter'+str(iteration+1)+'.pth')
            torch.save({'model':D.state_dict(), 'optimizer':d_optimizer.state_dict()},
                       '/home1/yujiali/cf_mri_2/bl_methods/HA_GAN_master/checkpoint/'+args.exp_name+'/D_iter'+str(iteration+1)+'.pth')
            torch.save({'model':E.state_dict(), 'optimizer':e_optimizer.state_dict()},
                       '/home1/yujiali/cf_mri_2/bl_methods/HA_GAN_master/checkpoint/'+args.exp_name+'/E_iter'+str(iteration+1)+'.pth')
            torch.save({'model':Sub_E.state_dict(), 'optimizer':sub_e_optimizer.state_dict()},
                       '/home1/yujiali/cf_mri_2/bl_methods/HA_GAN_master/checkpoint/'+args.exp_name+'/Sub_E_iter'+str(iteration+1)+'.pth')
