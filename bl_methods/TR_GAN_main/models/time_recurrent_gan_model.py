...
from bl_methods.TR_GAN_main.util.image_pool import ImagePool
...
class TimeRecurrentGanModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if True:
            parser.add_argument('--T_length', type=int, default=2, help='Number of sessions of input MRI image')
            ...
            parser.add_argument('--lambda_list', default=[1], help='weight for different session loss', type=float, nargs='+')
    ...
    def set_input(self, input, epoch=10000):
        ...
        for i in range(self.dataset_session_length):
            ...
            if self.swap and self.phase == 'train':
                image_swapped_list = []
                for idx in range(image_ori.shape[0]):
                    image_swapped_list.append(self.__swap_data__(image_ori[idx], N_patch=self.swap_patchs))
                    
                image_swapped = torch.cat(image_swapped_list, dim=0)
    ...
    def image_preprocess(self, data, crop_patchs=8, num_crop_D=10):
        ...
        if self.D_Preprocess == 'random_crop':
            ...
            for i in range(data.shape[0]):
                processed_list.append(self.__random_center_crop_multi__(data[i], num_crop_D=num_crop_D))
                
            return torch.cat(processed_list, dim=0)
        elif self.D_Preprocess == 'equality_crop':
            processed_list = []
            for i in range(data.shape[0]):
                processed_list.append(self.__equality_crop__(data[i], crop_patchs=crop_patchs))
            return torch.cat(processed_list, dim=0)
        else:
            return data
