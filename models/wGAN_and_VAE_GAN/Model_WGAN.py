...
class Discriminator(nn.Module):
    def __init__(self, channel=512):
        ...
        self.conv5 = nn.Conv3d(channel, channel*2, kernel_size=4, stride=2, padding=1)
        self.bn5 = nn.BatchNorm3d(channel*2)
        
        self.conv6 = nn.Conv3d(channel*2, n_class, kernel_size=4, stride=2, padding=1)
        
        self.linear = nn.Linear(2*3*2, 1)
    ...
    def forward(self, x, _return_activations=False):
        ...
        h6 = self.conv6(h5)
        #pdb.set_trace()
        output = self.linear(nn.Flatten()(h6))
