...
class Discriminator(nn.Module):
    def __init__(self, channel=512,out_class=1):
        ...
        self.conv5 = nn.Conv3d(channel, channel*2, kernel_size=4, stride=1, padding=0)
        self.bn5 = nn.BatchNorm3d(channel*2)
        self.conv6 = nn.Conv3d(channel*2, n_class, kernel_size=4, stride=1, padding=0)
        self.Linear = nn.Linear(4*6*4, 1)
    ...
    def forward(self, x):
        ...
        h6 = self.conv6(h5)
        output = F.sigmoid(self.Linear(h6.view(h6.size()[0],-1)))
        return output
...
class Encoder(nn.Module):
    def __init__(self, channel=512,out_class=1):
        ...
        self.conv0 = nn.Conv3d(1, channel//16, kernel_size=4, stride=2, padding=1)
        self.conv1 = nn.Conv3d(channel//16, channel//8, kernel_size=4, stride=2, padding=1)
    ...
    def forward(self, x, _return_activations=False):
        ...
        #pdb.set_trace()
        h0 = F.leaky_relu(self.conv0(x), negative_slope=0.2)
        h1 = F.leaky_relu(self.bn1(self.conv1(h0)), negative_slope=0.2)
        h2 = F.leaky_relu(self.bn2(self.conv2(h1)), negative_slope=0.2)
        h3 = F.leaky_relu(self.bn3(self.conv3(h2)), negative_slope=0.2)
        h4 = F.leaky_relu(self.bn4(self.conv4(h3)), negative_slope=0.2)
...
class Generator(nn.Module):
    def __init__(self, noise:int=100, channel:int=64):
        ...
        self.noise = noise
        self.fc = nn.Linear(1000,512*5*6*5)
        ...
        self.tp_conv5 = nn.Conv3d(_c, _c//2, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn5 = nn.BatchNorm3d(_c//2)
        
        self.tp_conv6 = nn.Conv3d(_c//2, 1, kernel_size=3, stride=1, padding=1, bias=False)
        
    def forward(self, noise):
        noise = noise.view(-1, 1000)
        h = self.fc(noise)
        h = h.view(-1,512,5,6,5)
        h = F.relu(self.bn1(h))
