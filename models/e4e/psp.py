...
class pSp(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.opts = opts
        # Define architecture
        self.encoder, self.noise_encoder = self.set_encoder()
    ...
    def set_encoder(self):
        if self.opts.encoder_type == 'GradualStyleEncoder':
            ...
            encoder = psp_encoders.Encoder4Editing3D(25, 'ir_se_3d', self.opts)
            noise_encoder = psp_encoders.NoiseEncoder3D()
        else:
            raise Exception('{} is not a valid encoders'.format(self.opts.encoder_type))
        return encoder, noise_encoder
