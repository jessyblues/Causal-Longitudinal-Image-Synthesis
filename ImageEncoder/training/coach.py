...
from datasets.images_dataset import MRI_Dataset
...
class Coach:
    def __init__(self, opts, prev_train_checkpoint=None):
        ...
        #self.net = torch.nn.DataParallel(self.net, device_ids=np.arange(torch.cuda.device_count()).tolist)
        ...
        if self.opts.w_discriminator_lambda > 0:
            self.discriminator = LatentCodesDiscriminator(128, 4).to(self.device)
        ...
        # Initialize logger
        self.log_dir = os.path.join(opts.exp_dir, 'log')
        os.makedirs(os.path.join(self.log_dir, 'train'), exist_ok=True)
        os.makedirs(os.path.join(self.log_dir, 'test'), exist_ok=True)
    ...
    def load_from_train_checkpoint(self, ckpt):
        ...
        #pdb.set_trace()
        if 'stylegan_weights' in self.opts.update_param_list:
            print('changing decoder weights to another pretrained! : {}'.format(self.opts.stylegan_weights))
            ckpt_stylegan = torch.load(self.opts.stylegan_weights)
            self.net.decoder.load_state_dict(ckpt_stylegan['generator'])
            #pdb.set_trace()
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                y, y_hat, latent = self.forward(batch)
                loss, encoder_loss_dict= self.calc_loss(y, y_hat, latent)
                ...
                if self.global_step % self.opts.image_interval == 0 or (
                    ...
                    self.parse_and_log_images(y, y_hat, subdir='train', display_count=1)
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            with torch.no_grad():
                y, y_hat, latent = self.forward(batch)
                loss, cur_encoder_loss_dict = self.calc_loss(y, y_hat, latent)
                cur_loss_dict = {**cur_loss_dict, **cur_encoder_loss_dict}
            agg_loss_dict.append(cur_loss_dict)

            # Logging related
            self.parse_and_log_images(y, y_hat, subdir='test', display_count=1)
    ...
    def configure_optimizers(self):
        ...
        if self.opts.noise_encoder:
            params += list(self.net.noise_encoder.parameters())
        else:
            self.requires_grad(self.net.decoder, False)
    ...
    def configure_datasets(self):
        ...
        train_dataset = MRI_Dataset(source_root=dataset_args['train_source_root'],
                                      target_root=dataset_args['train_target_root'],
                                      source_transform=transforms_dict['transform_source'],
                                      target_transform=transforms_dict['transform_gt_train'],
                                      opts=self.opts)
        test_dataset = MRI_Dataset(source_root=dataset_args['test_source_root'],
    ...
    def calc_loss(self, y, y_hat, latent):
        loss_dict = {}
        loss = 0.0
        if self.is_training_discriminator():  # Adversarial loss
            ...
            dims_to_discriminate = self.get_dims_to_discriminate() if self.is_progressive_training() else \
                list(range(self.net.decoder.module.n_latent))
        ...
        if self.opts.progressive_steps and self.net.encoder.module.progressive_stage.value != 12:  # delta regularization loss
            total_delta_loss = 0
        ...
        if self.opts.l1_lambda > 0:
            ...
            y_hat_ = torch.zeros_like(y_hat, dtype=torch.float,device=self.device)
            for i in range(y_hat.shape[0]):
                th = torch.min(y_hat[i])+35/256*(torch.max(y_hat[i])-torch.min(y_hat[i]))
                y_hat_[i] = (y_hat[i]>th)*y_hat[i]
            
            loss_l1 = F.l1_loss(y_hat_, y)
            loss_dict['loss_l1'] = float(loss_l1)
            loss += loss_l1 * self.opts.l1_lambda
        
        if self.opts.l1_fft_lambda > 0:
            
            loss_fft = .0
            y_hat__ = torch.zeros_like(y_hat, dtype=torch.float,device=self.device)
            
            for i in range(y_hat.shape[0]):
                th = torch.min(y_hat[i])+35/256*(torch.max(y_hat[i])-torch.min(y_hat[i]))
                y_hat__[i] = (y_hat[i]>th)*y_hat[i]

                fftn1 = torch.fft.fftn(y_hat__[i].squeeze())
                fftn2 = torch.fft.fftn(y[i].squeeze())

                dis = abs(fftn1 - fftn2)
                loss_fft += F.l1_loss(dis, torch.zeros_like(dis, dtype=torch.float, device=self.device))
            
            loss_fft_l1 = loss_fft/y_hat.shape[0]
            loss_dict['loss_fft_l1'] = float(loss_fft_l1)
            
            loss += loss_fft_l1 * self.opts.l1_fft_lambda
    ...
    def parse_and_log_images(self, y, y_hat, subdir, display_count=2):

        fig = plt.figure()
        for i in range(display_count):
            image = y[i, 0, y.shape[2]//2]
            image_ = y_hat[i, 0, y_hat.shape[2]//2]
            ax1 = fig.add_subplot(display_count, 2, 2*i+1)
            ax1.imshow(np.flip(image.detach().cpu().numpy(), axis=0), cmap=plt.get_cmap('gray'))
            ax1 = fig.add_subplot(display_count, 2, 2*i+2)
            ax1.imshow(np.flip(image_.detach().cpu().numpy(), axis=0), cmap=plt.get_cmap('gray'))
        plt.savefig(os.path.join(self.log_dir, subdir, 'sample_{}.jpg'.format(self.global_step)))
        plt.close()
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Discriminator ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    ...
    def sample_real_and_fake_latents(self, x):
        sample_z = torch.randn(self.opts.batch_size, 128, device=self.device)
