...
from torch.nn import Conv3d, BatchNorm3d, MaxPool3d, AdaptiveAvgPool3d
...
def get_blocks(num_layers):
    ...
    if num_layers == 32:
        blocks = [
            #get_block(in_channel=32, depth=64, num_units=2),
            #get_block(in_channel=64, depth=64, num_units=3),
            #get_block(in_channel=64, depth=128, num_units=8),
            #get_block(in_channel=128, depth=256, num_units=3)
            get_block(in_channel=16, depth=32, num_units=2),
            get_block(in_channel=32, depth=32, num_units=3),
            get_block(in_channel=32, depth=64, num_units=4),
        ...
        raise ValueError("Invalid number of layers: {}. Must be one of [30, 50, 100, 152]".format(num_layers))
    return blocks
...
class bottleneck_IR_3d(Module):
    def __init__(self, in_channel, depth, stride):
        ...
        self.res_layer = Sequential(
            ...
            Conv3d(in_channel, depth, (3, 3, 3), (1, 1, 1), 1, bias=False), PReLU(depth),
            Conv3d(depth, depth, (3, 3, 3), stride, 1, bias=False), BatchNorm3d(depth)
        )
