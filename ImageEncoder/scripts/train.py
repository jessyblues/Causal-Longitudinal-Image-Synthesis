...
def main():
	...
	opts.batch_size = torch.cuda.device_count()
	opts.test_batch_size = torch.cuda.device_count()
...
def load_train_checkpoint(opts):
	...
	previous_train_ckpt = torch.load(opts.resume_training_from_ckpt, map_location='cuda:0')
...
def create_initial_experiment_dir(opts):
	if os.path.exists(opts.exp_dir):
		print('Attention... {} already exists'.format(opts.exp_dir))
	else:
